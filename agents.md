...
- **Loading States & Visual Feedback:** Always use `useTransition` when calling Server Actions from buttons to provide immediate visual feedback (e.g., changing text to "Mengirim..." or "Memuat Data..." dan showing a spinner). Furthermore, for actions that take time (like filtering or programmatic `router.push` with searchParams), ensure loading states are visible so the user knows the app is not stuck. If the global top loading bar (TopLoader) doesn't trigger automatically on `router.push`, explicitly import `NProgress` from `nprogress` and call `NProgress.start()` before the push, and `NProgress.done()` when the `isPending` state completes.
